package java.main.functional.demo;
...
public class Comonad_example {
  public static void main(final String[] args) {
    for (final Stream<Character> p : perms(fromString("Hello").toStream())) {
    ...
    for (final Zipper<Character> z : fromStream(s))
      r = join(z.cobind(zp ->
            perms(zp.lefts().reverse().append(zp.rights())).map(
                F1Functions.o(Stream.<Character>cons().f(zp.focus()), P.<Stream<Character>>p1())
            )
      ).toStream());
    return r;
  }
